...
import EmailShare from '../certificates/EmailShare/EmailShare';
...
const CATEGORY_NAME_TRANSLATION_MAP = {
    [CATEGORY_IDS.SECRETS_MANAGEMENT]: 'translate(3570 2770)',
    [CATEGORY_IDS.DAST]: 'translate(3680 2560)',
    [CATEGORY_IDS.SAST]: 'translate(3958 2577)',
    [CATEGORY_IDS.SEVERLESS_SEC]: 'translate(4140 2770)',
    [CATEGORY_IDS.CONTAINER_SEC]: 'translate(4323 2570)',
    [CATEGORY_IDS.BUG_BOUNTY]: 'translate(3980 2565)',
    [CATEGORY_IDS.VULN_MANAGEMENT]: 'translate(4090 2770)',
    [CATEGORY_IDS.THREAT_MODEL]: 'translate(3970 2950)',
    [CATEGORY_IDS.CLOUD_SEC]: 'translate(4320 2915)',
    [CATEGORY_IDS.MOBILE_SEC]: 'translate(3957 2905)',
    [CATEGORY_IDS.SCA]: 'translate(3678 2930)',
};
const CATEGORY_DESCRIPTION_TRANSLATION_MAP = {
    [CATEGORY_IDS.SECRETS_MANAGEMENT]: 'translate(3570 2810)',
    [CATEGORY_IDS.DAST]: 'translate(3680 2600)',
    [CATEGORY_IDS.SAST]: 'translate(3958 2617)',
    [CATEGORY_IDS.SEVERLESS_SEC]: 'translate(4140 2810)',
    [CATEGORY_IDS.CONTAINER_SEC]: 'translate(4323 2610)',
    [CATEGORY_IDS.BUG_BOUNTY]: 'translate(3980 2605)',
    [CATEGORY_IDS.VULN_MANAGEMENT]: 'translate(4090 2810)',
    [CATEGORY_IDS.THREAT_MODEL]: 'translate(3970 2990)',
    [CATEGORY_IDS.CLOUD_SEC]: 'translate(4320 2945)',
    [CATEGORY_IDS.MOBILE_SEC]: 'translate(3957 2945)',
    [CATEGORY_IDS.SCA]: 'translate(3678 2970)',
};

class AcademyTourView extends Component {
    constructor(props) {
        super(props);
        this.titleLinesCount = 0;
        this.descLinesCount = 0;
    ...
    isActive = categoryId => {
        const { introCourses } = this.props;
        return introCourses.find(item => item.category.id === categoryId);
    }
    ...
    handleActiveCategory = categoryId => {
        this.setState({ activeCategory: categoryId, showPopup: true }, this.handleDescContent);
    }

    generateStringOnTags = (string, maxLetters = 50) => {
        const arr = string.split(' ');
        return arr.reduce((accumlator, current) => {
            if (current.length + accumlator[accumlator.length - 1].length < maxLetters) {
                const data = [...accumlator];
                data[data.length - 1] = !accumlator[0].length ? `${current}` : `${data[data.length - 1]} ${current}`;
                return data;
            }
            const data = [...accumlator];
            data.push(current);
    ...
    generateCategoryDescription = desc => {
        if (!desc) {
            this.descLinesCount = 0;
            this.setState({ categoryDescription: '' });
            return;
        }
        const descArr = this.generateStringOnTags(desc);
        this.descLinesCount = descArr.length - 1;
        const categoryDescription = (
            <>
                {descArr.map((des, i) => <tspan key={des + i} x="0" y={`${(i * 30) + (this.titleLinesCount * 30)}`}>{des}</tspan>)}
            </>
        );
        this.setState({ categoryDescription });
    }

    generateCategoryTitle = title => {
        const titleArr = this.generateStringOnTags(title, 45);
        this.titleLinesCount = titleArr.length - 1;
        return (
            <>
                {titleArr.map((e, i) => <tspan key={e + i} x="0" y={`${i * 30}`}>{e}</tspan>)}
            </>
        );
    }
    ...
    getCategoryTitle = () => {
        ...
        const { introCourses } = this.props;
        if (!introCourses.length) {
            return '';
        }
        const course = introCourses.find(e => e.category.id === activeCategory);
        if (course && course.title) {
            return this.generateCategoryTitle(course.title);
        }
        return '';
    }
    ...
    handleDescContent = () => {
        const { activeCategory } = this.state;
        const { introCourses } = this.props;
        introCourses.forEach(e => {
            if (e.category.id === activeCategory) {
                this.generateCategoryDescription(e.category.description);
            }
        });
    }
    ...
    handlePopup = activeCategory => {
        switch (activeCategory) {
        case CATEGORY_IDS.SECRETS_MANAGEMENT:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3020, 2510)' : 'matrix(1, 0, 0, 1, 3020, 2600)';
        case CATEGORY_IDS.DAST:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3130, 2300)' : 'matrix(1, 0, 0, 1, 3130, 2390)';
        case CATEGORY_IDS.SAST:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3408, 2317)' : 'matrix(1, 0, 0, 1, 3408, 2407)';
        case CATEGORY_IDS.SEVERLESS_SEC:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3590, 2510)' : 'matrix(1, 0, 0, 1, 3590, 2600)';
        case CATEGORY_IDS.CONTAINER_SEC:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3773, 2310)' : 'matrix(1, 0, 0, 1, 3773, 2400)';
        case CATEGORY_IDS.BUG_BOUNTY:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3430, 2305)' : 'matrix(1, 0, 0, 1, 3755, 2395)';
        case CATEGORY_IDS.VULN_MANAGEMENT:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3540, 2520)' : 'matrix(1, 0, 0, 1, 3860, 2610)';
        case CATEGORY_IDS.THREAT_MODEL:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3420, 2690)' : 'matrix(1, 0, 0, 1, 3742, 2830)';
        case CATEGORY_IDS.CLOUD_SEC:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3770, 2645)' : 'matrix(1, 0, 0, 1, 3770, 2805)';
        case CATEGORY_IDS.MOBILE_SEC:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3407, 2645)' : 'matrix(1, 0, 0, 1, 3407, 2805)';
        case CATEGORY_IDS.SCA:
            return this.isActive(activeCategory) ? 'matrix(1, 0, 0, 1, 3128, 2670)' : 'matrix(1, 0, 0, 1, 3128, 2820)';
        default:
            return '';
        }
    }

    handleComingSoon = activeCategory => {
        switch (activeCategory) {
        case CATEGORY_IDS.SECRETS_MANAGEMENT:
            return 'translate(3570 2850)';
        case CATEGORY_IDS.DAST:
            return 'translate(3680 2640)';
        case CATEGORY_IDS.SAST:
            return 'translate(3958 2657)';
        case CATEGORY_IDS.SEVERLESS_SEC:
            return 'translate(4140 2850)';
        case CATEGORY_IDS.CONTAINER_SEC:
            return 'translate(4323 2650)';
        case CATEGORY_IDS.BUG_BOUNTY:
            return 'translate(4305 2645)';
        case CATEGORY_IDS.VULN_MANAGEMENT:
            return 'translate(4410 2860)';
        case CATEGORY_IDS.THREAT_MODEL:
            return 'translate(4292 3080)';
        case CATEGORY_IDS.CLOUD_SEC:
            return 'translate(4320 3055)';
        case CATEGORY_IDS.MOBILE_SEC:
            return 'translate(3957 3055)';
        case CATEGORY_IDS.SCA:
            return 'translate(3678 3070)';
        default:
            return '';
        }
    }

    handleCategoryName = activeCategory => CATEGORY_NAME_TRANSLATION_MAP[activeCategory]

    handleCategoryDesc = activeCategory => CATEGORY_DESCRIPTION_TRANSLATION_MAP[activeCategory]
    ...
    getDynamicHeight = () => {
        const { activeCategory } = this.state;
        return this.isActive(activeCategory)
            ? this.titleLinesCount + this.descLinesCount > 3
                ? `${210 + (30 * (this.titleLinesCount + this.descLinesCount - 3))}`
                : '210'
            : '50';
    }
    ...
    render() {
        const {
            introCourses, certificate, sendCertificateToEmail, createActiveIntroductionCourse,
        } = this.props;
        const {
            activeCategory, showPopup, categoryDescription, emailModalVisible, showModal,
        } = this.state;
        return (
            <>
                {introCourses
                    ? (
                        <div className="academy-tour">
                            <div className="certificate-image-cont">
                                {
                                    certificate ? <img src={certificate.badge} onClick={this.showCertificate} alt="certificate 101" style={{ cursor: 'pointer' }} />
                                        : <img src="/img/certificate_101.png" alt="certificate 101" style={{ filter: 'opacity(0.5)' }} />
                                }
                            </div>
                                ...
                                >
                                    ...
                                    <g id="Group_434" data-name="Group 434" transform="translate(-3399.5 -2494.5)">
                                        <g id="Group_424" data-name="Group 424" transform="translate(-392 -2479)">
                                            <g transform="matrix(1, 0, 0, 1, 3791.5, 4973.5)" filter="url(#Path_317)">
                                                <path
                                                    id="Path_317-2"
                                                    data-name="Path 317"
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.SEVERLESS_SEC) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Ellipse_11)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.SEVERLESS_SEC)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.SEVERLESS_SEC)}
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.SECRETS_MANAGEMENT) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Ellipse_13)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.SECRETS_MANAGEMENT)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.SECRETS_MANAGEMENT)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <circle
                                                id="Ellipse_13-2"
                                        ...
                                        />
                                        <g
                                            className={this.isActive(CATEGORY_IDS.VULN_MANAGEMENT) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Path_308)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.VULN_MANAGEMENT)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.VULN_MANAGEMENT)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <path
                                                id="Path_308-2"
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.MOBILE_SEC) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Ellipse_10)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.MOBILE_SEC)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.MOBILE_SEC)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <circle
                                                id="Ellipse_10-2"
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.SAST) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Path_310)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.SAST)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.SAST)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <path
                                        ...
                                        <g
                                            ...
                                            id="Group_6"
                                            data-name="Group 6"
                                            transform="translate(3854.393 2650.59)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.SAST)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.SAST)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.CLOUD_SEC) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Path_307)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.CLOUD_SEC)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.CLOUD_SEC)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <path
                                                id="Path_307-2"
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.CONTAINER_SEC) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Path_311)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.CONTAINER_SEC)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.CONTAINER_SEC)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <path
                                                id="Path_311-2"
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.DAST) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Path_312)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.DAST)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.DAST)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <path
                                                id="Path_312-2"
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.BUG_BOUNTY) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Ellipse_12)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.BUG_BOUNTY)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.BUG_BOUNTY)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <circle
                                                id="Ellipse_12-2"
                                        ...
                                        />
                                        <g
                                            className={this.isActive(CATEGORY_IDS.THREAT_MODEL) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Ellipse_9)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.THREAT_MODEL)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.THREAT_MODEL)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <circle
                                                id="Ellipse_9-2"
                                        ...
                                        <g
                                            className={this.isActive(CATEGORY_IDS.SCA) ? 'icon_active' : 'icon_inactive'}
                                            transform="matrix(1, 0, 0, 1, 3399.5, 2494.5)"
                                            filter="url(#Path_309)"
                                            onClick={() => this.handleClick(CATEGORY_IDS.SCA)}
                                            onMouseEnter={() => this.handleActiveCategory(CATEGORY_IDS.SCA)}
                                            onMouseLeave={this.closePopUp}
                                        >
                                            <path
                                                id="Path_309-2"
                                        ...
                                        <path
                                            ...
                                            fill="#fff"
                                        />
                                        <g id="Group_433" data-name="Group 433" transform="translate(30)">
                                            {showPopup
                                    && (
                                        <g
                                            transform={this.handlePopup(activeCategory)}
                                            filter="url(#Rectangle_27)"
                                        >
                                            <rect
                                                id="Rectangle_27-2"
                                                data-name="Rectangle 27"
                                                width={this.isActive(activeCategory) ? '500' : '170'}
                                                height={this.getDynamicHeight()}
                                                rx="4"
                                                transform="translate(520 215.7)"
                                                fill="#fff"
                                            />
                                        </g>
                                    )}
                                            {showPopup && this.isActive(activeCategory)
                                                ? (
                                                    <g id="Group_432" data-name="Group 432" transform="translate(0)">
                                                        <text
                                                            id="CATEGORY_DESCRIPTION"
                                                            data-name="CATEGORY_DESCRIPTION"
                                                            transform={this.handleCategoryDesc(activeCategory)}
                                                            fill="#032140"
                                                            fontSize="30"
                                                        ...
                                                        <text
                                                            ...
                                                            transform={this.handleCategoryName(activeCategory)}
                                                            fill="#032140"
                                                            fontSize="24"
                                                ...
                                                : showPopup ? (
                                                    <g id="Group_432" data-name="Group 432" transform="translate(-17)">
                                                        <text
                                                            ...
                                                            transform={this.handleComingSoon(activeCategory)}
                                                            fill="#032140"
                                                            fontSize="24"
                                                            fontFamily="Montserrat-SemiBold, Montserrat"
                                                            fontWeight="600"
                                    ...
                                    <g id="Group_312" data-name="Group 312" transform="translate(4619.5 2848.198)">
                                        <path
                                            id="Path_41"
                                            data-name="Path 41"
                                            d="M388.935,399.719h-.684v-2.726a5.5,5.5,0,0,0-10.987,0v2.726h-.683a1.692,1.692,0,0,0-1.694,1.693v8.077a1.692,1.692,0,0,0,1.694,1.693h12.359a1.693,1.693,0,0,0,1.694-1.693v-8.077a1.693,1.693,0,0,0-1.694-1.693m-10.186-2.726a4.011,4.011,0,0,1,8.017,0v4.213h2.169a.213.213,0,0,1,.208.207v8.077a.213.213,0,0,1-.208.207H376.58a.212.212,0,0,1-.208-.207v-8.077a.212.212,0,0,1,.208-.207h2.169Zm7.124-.126a3.129,3.129,0,0,0-6.233,0v4.339h6.233Zm-4.747,0a1.656,1.656,0,0,1,3.261,0v2.853H381.13Zm1.636,5.231a2.161,2.161,0,0,0-1.694,3.513v1.033a1.69,1.69,0,1,0,3.38,0V405.61a2.161,2.161,0,0,0-1.687-3.513m.943,3.224v1.322a.947.947,0,1,1-1.894,0v-1.322a1.422,1.422,0,1,1,1.894,0m24.724-1.679a2.928,2.928,0,0,0-2.838,2.192h-8.469a14.757,14.757,0,0,0,.505-3.271h3.21a2.935,2.935,0,1,0,0-1.486h-3.21a14.451,14.451,0,0,0-.408-2.734H405.6a2.934,2.934,0,1,0,0-1.487h-8.815a14.954,14.954,0,0,0-8.789-8.915v-8.685a2.928,2.928,0,1,0-1.486,0v8.217a14.792,14.792,0,0,0-3.261-.475v-3a2.934,2.934,0,1,0-1.486,0v3.024a14.487,14.487,0,0,0-2.734.445v-8.217a2.935,2.935,0,1,0-1.486,0v8.695a14.953,14.953,0,0,0-8.788,8.915H359.92a2.935,2.935,0,1,0,0,1.487h8.373a14.73,14.73,0,0,0-.431,3.261h-3.187a2.934,2.934,0,1,0,0,1.486h3.231a15.177,15.177,0,0,0,.483,2.742H359.92a2.928,2.928,0,1,0,0,1.486h8.967a14.936,14.936,0,0,0,8.64,8.514v9.087a2.928,2.928,0,1,0,1.486,0V416.3a14.753,14.753,0,0,0,3.261.475v3.389a2.934,2.934,0,1,0,1.486,0v-3.418a14.4,14.4,0,0,0,2.734-.446v8.618a2.935,2.935,0,1,0,1.486,0V415.83a14.934,14.934,0,0,0,8.64-8.514h8.967a2.932,2.932,0,1,0,2.838-3.678m-4.754-3.271a1.449,1.449,0,1,1-1.448,1.448,1.448,1.448,0,0,1,1.448-1.448m4.754-4.22a1.444,1.444,0,1,1-1.028.425,1.444,1.444,0,0,1,1.028-.425m-51.343,2.9a1.449,1.449,0,1,1,1.448-1.449,1.448,1.448,0,0,1-1.448,1.449m4.754,4.747a1.447,1.447,0,1,1,1.03-.423,1.447,1.447,0,0,1-1.03.423m-4.754,4.22a1.446,1.446,0,1,1,1.448-1.441,1.446,1.446,0,0,1-1.448,1.441m28.716-31.6a1.445,1.445,0,1,1,1.441,1.441,1.445,1.445,0,0,1-1.441-1.441m-4.755,4.747a1.448,1.448,0,1,1,.422,1.028,1.449,1.449,0,0,1-.422-1.028m-4.221-4.747a1.445,1.445,0,1,1,.426,1.025,1.445,1.445,0,0,1-.426-1.025m2.89,51.336a1.445,1.445,0,1,1-1.441-1.448,1.445,1.445,0,0,1,1.441,1.448M384.466,423a1.448,1.448,0,1,1-1.448-1.441A1.448,1.448,0,0,1,384.466,423m4.22,4.754a1.445,1.445,0,1,1-.423-1.025,1.445,1.445,0,0,1,.423,1.025m-5.928-12.444a9.673,9.673,0,1,1,.005,0m25.668-7.3a1.446,1.446,0,1,1,1.441-1.441,1.446,1.446,0,0,1-1.441,1.441m-19.494-8.3h-.684v-2.726a5.5,5.5,0,0,0-10.987,0v2.726h-.683a1.692,1.692,0,0,0-1.694,1.693v8.077a1.692,1.692,0,0,0,1.694,1.693h12.359a1.693,1.693,0,0,0,1.694-1.693V401.41a1.693,1.693,0,0,0-1.694-1.693m-10.186-2.726a4.011,4.011,0,0,1,8.017,0V401.2h2.169a.213.213,0,0,1,.208.207v8.077a.213.213,0,0,1-.208.207H376.582a.212.212,0,0,1-.208-.207v-8.077a.212.212,0,0,1,.208-.207h2.169Zm7.124-.126a3.129,3.129,0,0,0-6.233,0V401.2h6.233Zm-4.747,0a1.656,1.656,0,0,1,3.261,0v2.853h-3.264Zm1.636,5.231a2.161,2.161,0,0,0-1.694,3.513v1.033a1.69,1.69,0,1,0,3.38,0v-1.033a2.161,2.161,0,0,0-1.687-3.513m.943,3.224v1.322a.947.947,0,0,1-1.894,0v-1.32a1.408,1.408,0,0,1-.476-1.055,1.423,1.423,0,1,1,2.37,1.055m-.943-11.307a3.034,3.034,0,0,0-3.12,2.853v4.339h6.233v-4.339a3.031,3.031,0,0,0-3.112-2.853m-1.636,2.853a1.656,1.656,0,0,1,3.261,0v2.853h-3.264Zm1.636,5.231a2.161,2.161,0,0,0-1.694,3.513v1.033a1.69,1.69,0,1,0,3.38,0V405.61a2.161,2.161,0,0,0-1.687-3.513m.943,3.224v1.322a.947.947,0,0,1-1.894,0v-1.322a1.421,1.421,0,1,1,1.894,0m5.231-5.6h-.684v-2.726a5.5,5.5,0,0,0-10.987,0v2.726h-.683a1.692,1.692,0,0,0-1.694,1.693v8.077a1.692,1.692,0,0,0,1.694,1.693h12.359a1.693,1.693,0,0,0,1.694-1.693v-8.077a1.693,1.693,0,0,0-1.694-1.693m-10.186-2.726a4.011,4.011,0,0,1,8.017,0v4.213h2.169a.213.213,0,0,1,.208.207v8.077a.213.213,0,0,1-.208.207H376.587a.212.212,0,0,1-.208-.207v-8.077a.212.212,0,0,1,.208-.207h2.169Zm7.124-.126a3.129,3.129,0,0,0-6.233,0v4.339h6.233Zm-4.747,0a1.656,1.656,0,0,1,3.261,0v2.853h-3.261Zm1.636,5.231a2.161,2.161,0,0,0-1.694,3.513v1.033a1.69,1.69,0,1,0,3.38,0v-1.033a2.161,2.161,0,0,0-1.687-3.513m.943,3.224v1.322a.947.947,0,0,1-1.894,0v-1.322a1.408,1.408,0,0,1-.476-1.055,1.423,1.423,0,1,1,2.37,1.055m-.943-11.307a3.034,3.034,0,0,0-3.121,2.853v4.339h6.233v-4.339a3.031,3.031,0,0,0-3.112-2.853m-1.636,2.853a1.656,1.656,0,0,1,3.261,0v2.853h-3.261Zm1.636,5.231a2.161,2.161,0,0,0-1.694,3.513v1.033a1.69,1.69,0,1,0,3.38,0v-1.033a2.161,2.161,0,0,0-1.687-3.513m.943,3.224v1.322a.947.947,0,1,1-1.894,0v-1.322a1.408,1.408,0,0,1-.476-1.055,1.423,1.423,0,1,1,2.37,1.055"
                                    ...
                                    <g id="Group_310" data-name="Group 310" transform="translate(4229.5 3058.198)">
                                        <path
                                            id="Path_45"
                                            data-name="Path 45"
                                            d="M904.644,75.511a13.267,13.267,0,0,0-1.956-5.289m.759,12.975a13.224,13.224,0,0,0,1.318-5.288m-4.024-10.084a15.791,15.791,0,0,0-9.323-3.8h-.925a21.481,21.481,0,0,0-42.808.38c-2.314.555-2.277.12-6.157,3.424-6.667,6.088-3.677,15.654-1.235,17.769,1.542,1.871,6.258,5.17,10.538,5.17H891.42a13.326,13.326,0,0,0,9.452-3.923c.392-.394-1.375,2.065,1.079-1.247,5.05-6.5,2.46-15.069-1.213-17.769M837.483,77.908A13.254,13.254,0,0,0,838.8,83.2m.764-12.975a13.174,13.174,0,0,0-1.96,5.289m61.566,9.633a10.93,10.93,0,0,1-7.751,3.217H850.833a10.96,10.96,0,0,1-7.744-18.729,10.818,10.818,0,0,1,5.939-3.062l.977-.158.029-.989a19.083,19.083,0,0,1,38.141-.161l.044,1.151h3.2a10.966,10.966,0,0,1,7.756,18.727"
                                    ...
                                    <g id="Group_315" data-name="Group 315" transform="translate(4237.5 2648.075)">
                                        <path
                                            id="Path_56"
                                            data-name="Path 56"
                                            d="M789.666,221.154a2.756,2.756,0,0,0-2.056,4.547v1.558a2.056,2.056,0,1,0,4.113,0V225.7a2.772,2.772,0,0,0,.659-1.79,2.741,2.741,0,0,0-2.716-2.757m.829,3.684-.244.223v2.194a.583.583,0,1,1-1.165,0v-2.191l-.243-.223a1.241,1.241,0,1,1,1.652,0"
                                    ...
                                    <path
                                        ...
                                        fill="#fff"
                                    />
                                    <g id="Group_433" data-name="Group 433" transform="translate(-0.072)">
                                        {showPopup
                                && (
                                    <g
                                        transform={this.handlePopup(activeCategory)}
                                    ...
                                    >
                                        <rect
                                            id="Rectangle_27-2"
                                            data-name="Rectangle 27"
                                            width={this.isActive(activeCategory) ? '470' : '200'}
                                            height={this.getDynamicHeight()}
                                            rx="4"
                ...
                />
                {
                    ...
                    && (
                        <Modal
                            title="Introduction Certificate"
                            visible={showModal}
                            width="50%"
                            onCancel={this.showCertificate}
                            footer={[]}
                        >
                ...
                <EmailShare
                    ...
                    certificateToShare={certificate ? certificate.badge : null}
...
export { AcademyTourView };
export default withRouter(AcademyTourView);
