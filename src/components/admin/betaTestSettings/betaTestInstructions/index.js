...
class BetaTestInstructions extends Component {
    ...
    componentDidMount() {
        const loader = message.loading('load beta test instructions');
        this.props.getBetaTestInstructions().then(res => {
            if (res.status === 200) {
                this.setState({
    ...
    save = () => {
        ...
        this.props.updateBetaTestInstructions({
            content: steps,
        }).then(res => {
            if (res.status === 200) {
                this.setState({
    ...
    render() {
        const { selectedIndex, selectedStep, steps } = this.state;
        return (
            <div className="beta-test-container">
                <Title className="beta-test-title" level={4}> Beta Test Instructions </Title>

                <div className="beta-test-list">
                    <Row>
                        <Col offset={11} span={13}>
                            <div className="save-beta-test-container">
                                <Button
                                    className="btn-save-beta-test"
                                    onClick={this.save}
                                    type="primary"
                                >
                                    Save
                                ...
                                <Button
                                    className="btn-preview-beta-test"
                                    type="primary"
                                    disabled={!this.state.steps.length}
...
export default BetaTestInstructions;
